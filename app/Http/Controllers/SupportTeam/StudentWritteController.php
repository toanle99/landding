    ...
    */
        ...
        if(Qs::userIsTeamSAT()){
            ...
            if(Qs::userIsTeacher()){
                ...
                foreach($cl_tcs as $cl_tc)
                    $student = array_merge($student,$this->student->findStudentIdsByClasses($cl_tc));        
    ...
    public function update_st($gxp_id, $status)
    {
        ...
        if(count($data) >0) {
            ...
            // // send mail to teacher when parent accept status=1
            if($data['status'] == 1) {
                $student = $this->student->getRecord(['id' => $gxp->student_record_id])->first();
                $s = SettingRepo::all();
                $system = $s->flatMap(function($s){
                    return [$s->type => $s->description];
                });
                // $system_email = $system['system_email'].'';
                 
                $message = [
                    'system_name' => $system['system_name'],
                    'system_footer' => $system['system_footer'],
                    'type' => 'Giấy xin phép từ '. $student->user ->name,
                    'task' => 'Xin nghỉ '.$gxp->session_time.', ngày '.date('d-m-Y', strtotime($gxp->date_at)),
                    'content' => ' Lý do:  '.$gxp->reason,
                ];
                $title = 'Giấy xin phép từ '. $student->user ->name;
                //// send           // message  // from               // to email                         // tiêu đều
                try {
                    SendEmail::dispatch($message, $system['system_email'], $student->my_class->teacher->email, $title)->delay(now());
                }
                catch(Exception $e) { 
