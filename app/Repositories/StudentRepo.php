...
class StudentRepo {
    ...
    {
        return $this->activeStudents()->where(['my_class_id' => $class_id])->get()->pluck('id')->sortBy('id')->toArray();
    }
    ...
    public function getRecordByUserID($ids)
    {
        return $this->activeStudents()->where('user_id', $ids)->with('user');
    }
